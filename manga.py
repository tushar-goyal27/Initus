...
class MANGA(commands.Cog):
    ...
    @commands.command(name = 'manga', help='_manga "name of the manga" to get the info about the anime')
    async def manga(self, ctx, keyword = ''):
        ...
        await channel.send(f'manga command used by { self.de_emojify(ctx.author) } for keyword { keyword } in { ctx.channel }')
        ...
        url = f'https://myanimelist.net/search/prefix.json?type=manga&keyword={ keyword }&v=1'
